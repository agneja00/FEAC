...
router.put("/:lang/user/:email", authMiddleware, upload.single("photo"), async (req, res) => {
  const lang = validLanguages.includes(req.params.lang) ? req.params.lang : defaultLanguage;
  const currentEmail = req.params.email;
  const { name, email: newEmail, age, country, city, password } = req.body;

  try {
    const updateData: any = { name, email: newEmail || currentEmail, age, country, city };
    ...
    if (req.file) {
      ...
      const result = await new Promise<any>((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "avatars", public_id: `${newEmail || currentEmail}_avatar`, overwrite: true },
